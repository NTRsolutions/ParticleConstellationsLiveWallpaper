...
final class SceneController implements Runnable, ParticlesScene {
    ...
    void inflate(@NonNull final Resources r, @NonNull final AttributeSet attrs) {
        final TypedArray a = r.obtainAttributes(attrs, com.doctoror.particlesdrawable.R.styleable.ParticlesView);
        try {
            handleAttrs(a);
        } finally {
            a.recycle();
    ...
    void handleAttrs(@NonNull final TypedArray a) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_minDotRadius) {
                minDotRadius = a.getDimension(attr, Defaults.DEFAULT_MIN_DOT_RADIUS);

            } else if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_maxDotRadius) {
                maxDotRadius = a.getDimension(attr, Defaults.DEFAULT_MAX_DOT_RADIUS);

            } else if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_lineThickness) {
                setLineThickness(a.getDimension(attr, Defaults.DEFAULT_LINE_THICKNESS));

            } else if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_lineDistance) {
                setLineDistance(a.getDimension(attr, Defaults.DEFAULT_LINE_DISTANCE));

            } else if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_numDots) {
                setNumDots(a.getInteger(attr, Defaults.DEFAULT_DOT_NUMBER));

            } else if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_dotColor) {
                setDotColor(a.getColor(attr, Defaults.DEFAULT_DOT_COLOR));

            } else if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_lineColor) {
                setLineColor(a.getColor(attr, Defaults.DEFAULT_LINE_COLOR));

            } else if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_frameDelayMillis) {
                setFrameDelay(a.getInteger(attr, Defaults.DEFAULT_DELAY));

            } else if (attr == com.doctoror.particlesdrawable.R.styleable.ParticlesView_stepMultiplier) {
